...
//
// August 2007:
// Updated by Raul Arrabales. Branch MazeSimulatorRA created.
// RA version removes LRF and include Simulated SONAR.
// Namespace and contract ID are changed in order to not interfere with original MazeSimulator.
//
// August 2007:
// Updated Again by Raul Arrabales. Followed the suggestion from Trevor:
// Boolean flags have been added to the State, so both LRF and Sonar
// can be used with the same main code.
// 
...
// Raul - We can chose to use LRF or Sonar
...
// Raul - Dec 2007 - Using my Simulated P3DX bumper.
// using bumper = Microsoft.Robotics.Services.Simulation.Sensors.Bumper.Proxy;
using bumper = ConsciousRobots.Cranium.Simulation.Sensors.Bumper;
using pxbumper = ConsciousRobots.Cranium.Simulation.Sensors.Bumper.Proxy;

// Raul - Jan 2010 - Required for GPS support
using GPS = Cranium.Simulation.Sensors.SimulatedPioneerGPS;
...
using pursuitcam = Microsoft.Robotics.Services.PursuitCamera;
using pursuitcamEnt = Microsoft.Robotics.Entities.PursuitCamera;
...
// Raul
using Cranium.Simulation.Sensors;



namespace Cranium.Simulation.Worlds
{
    [DisplayName("MazeSimulatorRA")]
    [Description("The MazeSimulator Service (RA version)")]
    ...
    public class MazeSimulatorService : DsspServiceBase
    {
        ...
        public const string InitialStateUri = ServicePaths.MountPoint + @"/packages/crubots/Config/MazeSimulatorRA.Config.xml";
        ...
        protected override void Start()
        {
            ...
            if (_state.Maze == null || _state.Maze == "")
                _state.Maze = "packages/crubots/Simulation/Worlds/MazeSimulatorRA/OfficeRaul1.bmp";
...
//                cBoxShape = new BoxShapeProperties(0, new Pose(), dimensions);
                ...
                {                                             
                    cBoxShape.TextureFileName = "../../packages/crubots/Simulation/Worlds/MazeSimulatorRA/Textures/bricks_4.jpg";
                }
                if (color == BasicColor.Red)
                {
                    cBoxShape.TextureFileName = "../../packages/crubots/Simulation/Worlds/MazeSimulatorRA/Textures/wood2.jpg";
                }
        ...
        void AddPioneer3DXRobot(Vector3 position)
        {
            ...
            // Raul - Dec 2007 - Use my simulated bumper instead
            // BumperArrayEntity bumperArray = CreateBumperArray();
            bumper.PioneerBumperEntity bumperArray = CreateBumperArray();
            ...
            // Insert pursuit cam
            pursuitcamEnt.PursuitCameraEntity pursuitCam = this.CreatePursuitCamera();
            SimulationEngine.GlobalInstancePort.Insert(pursuitCam);
        ...
        private VisualEntity CreateGPSEntity()
        {
            ...
            LogInfo("GPS: " + pxGPS.Contract.Identifier);
        ...
        /// <summary>
        /// Created a pursuit camera attached to the Pioneer chasis
        /// </summary>
        /// <returns></returns>
        private pursuitcamEnt.PursuitCameraEntity CreatePursuitCamera()
        {

            pursuitcamEnt.PursuitCameraEntity purCamEnt = new pursuitcamEnt.PursuitCameraEntity("P3DXMotorBase");
            purCamEnt.State.Name = "PursuitCam";
            purCamEnt.IsRealTimeCamera = true;

            //pursuitcam.Contract.CreateService(
            //    ConstructorPort,
            //    Microsoft.Robotics.Simulation.Partners.CreateEntityPartner(
            //    "http://localhost/" + purCamEnt.State.Name)
            //);
